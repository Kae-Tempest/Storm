...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not posts:
            self.stdout.write(
                self.style.ERROR('Aucun post trouvé. Veuillez d\'abord exécuter python manage.py seed_posts'))
            return
        ...
        for post in posts:
            ...
            for _ in range(num_comments):
                Comment.objects.create(
                    ...
                    created_at=post.created_at + timedelta(
                        hours=random.randint(1, 24 * 7),  # Commentaire dans la semaine qui suit le post
            ...
            if num_comments > 0:
                self.stdout.write(f'Créé {num_comments} commentaires pour le post {post.id}')

        self.stdout.write(self.style.SUCCESS(f'{comments_created} commentaires créés avec succès !'))